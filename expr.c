...
static ASTKind token_to_ast(TokenKind kind) {
    ...
    default: fprintf(stderr,"syntax error on line %d: expected operator\n",source_line); exit(EXIT_FAILURE);
    }
}
...
static int token_precedence(TokenKind kind) {
    ...
    case TOK_STAR: case TOK_SLASH: return 20;
    case TOK_EOF: return 0;
    default: fprintf(stderr,"syntax error on line %d: invalid operator\n",source_line); exit(EXIT_FAILURE);
    }
}
...
static ASTNode *primary(void) {
    ...
    if (current_token.kind != TOK_INT) {
        fprintf(stderr,"syntax error on line %d: expected integer\n",source_line);
        exit(EXIT_FAILURE);
    }
...
ASTNode *parse_expression(int previous_precedence) {
    ...
    ASTNode *right;
    TokenKind operator_token=current_token.kind;
    if (operator_token==TOK_EOF) return left;
    int precedence=token_precedence(operator_token);
    while (precedence > previous_precedence) {
        scan_token(&current_token);
        right=parse_expression(precedence);
        left=make_ast_node(token_to_ast(operator_token),left,right,0);
